...
func (gp *xwingPhys) update(dt float64, ctrl pixel.Vec, platforms []platform) {
	// apply controls
	switch {
	case ctrl.X < 0:
...
func (ga *xwingAnim) update(dt float64, phys *xwingPhys) {
	ga.counter += dt
	...
	// set the facing direction of the xwing
	if phys.vel.X != 0 {
		if phys.vel.X > 0 {
			ga.dir = +1
		} else {
...
func (ga *xwingAnim) draw(t pixel.Target, phys *xwingPhys) {
	if ga.sprite == nil {
...
func run() {
	...
	for !win.Closed() {
		...
		// lerp the camera position towards the xwing
